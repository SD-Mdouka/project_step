...
function App({ currentSteps }) {
  const state = currentSteps;
  return (
    <div className="App">
      <Card>
        <Container>
...
const FormStps = (props) => {
  ...
  const name = childrenArr[props.state - 1].props.name;
  const isErrors = props.errors[name] ? true : false;
  const isTouched = props.touched[name] ? true : false;
  const isEmpty =
    childrenArr[props.state - 1].props.value !== " " &&
    !childrenArr[props.state - 1].props.value
      ? true
      : false;
  const goBack = () => {
    setStep(props.state - 1);
  };
  const goNext = () => {
  ...
  return (
    <>
      ...
      {props.state < childrenArr.length && (
        <Button
          disabled={props.isSubmitting}
          variant="contained"
          onClick={goNext}
